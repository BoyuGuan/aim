...
def calculate_quantsim_accuracy(model: torch.nn.Module, evaluator: aimet_common.defs.EvalFunction,
                                use_cuda: bool = False, bit_width: int = 8) -> float:
    """
    ...
    :param bit_width: bit width after quantization
    ...
    quantsim = QuantizationSimModel(model=model, quant_scheme='tf_enhanced',
                                    ...
                                    default_output_bw=bit_width, default_param_bw=bit_width, in_place=False)
...
def apply_bias_correction(model: torch.nn.Module, data_loader: torch_data.DataLoader, bit_width: int = 8):
    """
    ...
    num_bias_correct_samples = 500

    params = QuantParams(weight_bw=bit_width, act_bw=bit_width, round_mode=rounding_mode, quant_scheme='tf_enhanced')
...
def cle_bc_example(_config: argparse.Namespace):
    """
    ...
    :param _config: This argparse.Namespace _config expects following parameters:
    ...
    data_pipeline = ImageNetDataPipeline(_config)
    ...
    accuracy = data_pipeline.evaluate(model, use_cuda=_config.use_cuda)
    logger.info("Original Model Top-1 accuracy = %.2f", accuracy)

    accuracy = calculate_quantsim_accuracy(model=model, evaluator=data_pipeline.evaluate, use_cuda=_config.use_cuda, bit_width=_config.bit_width)
    logger.info("Quantized Model Top-1 Accuracy Before CLE = %.2f", accuracy)
    # Applying cross-layer equalization (CLE)
    # Note that this API will equalize the model in-place

    time_before_cle  = time.time()
    apply_cross_layer_equalization(model=model, input_shape=(1, 3, 32, 32))
    time_after_cle  = time.time()
    logger.info('CLE time is ' + str(time_after_cle - time_before_cle))


    # Calculate quantized (INT8) accuracy after CLE
    accuracy = calculate_quantsim_accuracy(model=model, evaluator=data_pipeline.evaluate, use_cuda=_config.use_cuda, bit_width=_config.bit_width)
    logger.info("Quantized Model Top-1 Accuracy After CLE = %.2f", accuracy)
    ...
    time_before_bs  = time.time()
    apply_bias_correction(model=model, data_loader=data_loader,bit_width=_config.bit_width)
    time_after_bs  = time.time()
    logger.info('bs time is ' + str(time_after_bs - time_before_bs))
    logger.info( 'CLE-BS time is ' + str(time_after_bs - time_before_bs + time_after_cle - time_before_cle) )

    # Calculating accuracy on Quant Simulator
    accuracy = calculate_quantsim_accuracy(model=model, evaluator=data_pipeline.evaluate, use_cuda=_config.use_cuda, bit_width=_config.bit_width)
    logger.info("Quantized Model Top-1 Accuracy After Bias Correction = %.2f", accuracy)

    # Save the quantized model
    torch.save( model, os.path.join(_config.logdir,  "resnet_model_cle_bc.pth") )
...
if __name__ == '__main__':
    ...
    modelNames = ['resnet18', 'resnet50', 'mobilenet_v2']
    _config.logdir = os.path.join("benchmark_output", 'cle_bc', modelNames[_config.model] +  "_" + str(_config.bit_width) + "_" +  datetime.now().strftime("%Y-%m-%d-%H-%M-%S"))
    os.makedirs(_config.logdir, exist_ok=True)

    fileHandler = logging.FileHandler(os.path.join(_config.logdir, modelNames[_config.model]+"_cleBC.log"))
